...
Status check_capacity(EncodeInfo *encInfo)// Function to check if the source image has enough capacity to hold the secret file
{
    ...
    if (encInfo->image_capacity < required)
    {
        ...
        printf("ERROR: Image size insufficient foe encoding\n");
        return e_failure;
    }
    return e_success;
}
...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    {
        fprintf(stderr, "ERROR: Source image file must be a .bmp file\n");
        return e_failure;
    }
   ...
   /*/ char *dot = strrchr(argv[3], '.');
    if (dot != NULL)
        encInfo->extn_secret_file = dot;
    else
        encInfo->extn_secret_file = "";*/

    // Validate or set stego image file
    if (argv[4] == NULL)
    {
